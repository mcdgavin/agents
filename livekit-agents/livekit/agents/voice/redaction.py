...
class Redactor(Protocol):
    """Pluggable PII detector applied to text crossing a trust-boundary egress.

    Implementations MUST be idempotent: content may pass through ``redact()``
    more than once (e.g. a chat context redacted for the LLM sink is redacted
    again for the telemetry sink), so running it on its own output must leave
    the text unchanged. Placeholder replacements such as ``[CREDIT_CARD]``
    satisfy this naturally; transformations that re-match their own output
    (wrapping, hashing, partial masking of digits) do not.
    """
...
async def redact_chat_ctx(
    ...
    Redacts the text content of messages and function call outputs. Function
    call *arguments* are redacted for every sink except LLM: they are generated
    by the model, so for the LLM sink they round-trip back to the model that
    produced them, but for at-rest sinks they can echo PII the model saw (e.g.
    a card number passed to a payment tool). If the redactor raises or exceeds
    ``opts.timeout``, the affected text is replaced with
    :data:`REDACTION_FAILURE_MARKER`.
    """
    items = []
    for item in chat_ctx.items:
        ...
        elif item.type == "function_call" and sink is not RedactionSink.LLM:
            ctx = RedactionContext(sink=sink, role="tool")
            new_arguments = await _redact_text(item.arguments, opts, ctx)
            if new_arguments != item.arguments:
                item = item.model_copy(update={"arguments": new_arguments})
